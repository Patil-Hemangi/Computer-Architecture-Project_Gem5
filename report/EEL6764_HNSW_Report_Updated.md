...
**Main contribution:** This project shows that HNSW vector search on gem5 is primarily limited by serial DRAM-latency stalls caused by pointer-chasing in graph traversal. Across a broad design sweep, conventional hardware tuning such as larger ROBs, prefetching, and bandwidth-oriented memory upgrades provides little benefit, while software-side locality and compression changes deliver the best performance-per-cost tradeoff. The strongest result is BFS graph reordering plus scalar quantization, which improves IPC by 10.2% at zero additional hardware area.
...
This study is derived from the real SIFT-1M benchmark, which contains a much larger base-vector corpus and a 10,000-query set. Because gem5 simulation is extremely slow, the detailed microarchitectural evaluation uses a reduced subset of 500 base vectors and 20 sampled queries. This reduction is a practical simulation constraint rather than a change in workload type. The key HNSW execution behavior remains the same: irregular graph traversal creates data-dependent pointer-chasing, while distance computation remains a regular vector loop. Therefore, the absolute IPC, miss-rate, and DRAM-traffic values reported here should be interpreted as simulation-scale measurements, while the main architectural conclusion is expected to generalize more broadly: HNSW is dominated by memory-latency stalls from serialized graph traversal, and conventional hardware scaling provides limited benefit unless locality is improved or data size is reduced.
...
### 5.10 Predicted vs Observed Summary

| Intervention | Prediction from Analysis | Observed Result | Conclusion |
|---|---|---|---|
| Larger L2 | Small to moderate IPC gain; little MPKI reduction | Up to +10.6% IPC, miss rate changes only slightly | Capacity helps somewhat, but locality remains the main issue |
| Larger ROB | Minimal benefit | +0.7% at ROB=256 | More window size cannot break serial load dependence |
| HW stride prefetcher | Little or no benefit | 0.0% added gain | Regular prefetching cannot help irregular pointer-chasing |
| HBM / DDR5 | Bandwidth-focused memory should not help much | HBM: 0.0%, DDR5: -10.1% | Workload is latency-bound, not bandwidth-bound |
| More MSHRs | Minimal gain | 0.0% | MLP is fundamentally limited by inter-hop dependence |
| Graph BFS reorder | Some locality improvement | +1.8% IPC | Better layout helps cache behavior |
| Scalar quantization | Reduced DRAM bytes should help | +3.7% IPC | Compression improves effective cache capacity |
| Reorder + Quant | Best software-side PPA tradeoff | +10.2% IPC at 1.00x area | Best overall design choice in this study |
